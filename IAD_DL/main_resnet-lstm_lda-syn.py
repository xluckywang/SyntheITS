...
early_stopping = EarlyStopping(monitor='loss',  # 监控的指标是验证集的损失
                               patience=200,  # 如果在 10 个 epoch 内都没有改善就停止训练
                               verbose=1,  # 显示详细信息
...
def main():
    ...
    for RATIO in RATIOS:
        ...
        # 更新参数网格
        param_grid = {
            'solver': ['svd', 'lsqr', 'eigen'],  # 求解方法，可以选择'svd', 'lsqr', 'eigen'
            'shrinkage': [None],  # 收缩方法，这里设为None，让模型自动选择合适的协方差矩阵估计方法
            'priors': [None],  # 类别先验概率，可以选择None或者数组，这里设为None，表示不使用先验概率
            'n_components': [None, 1],  # LDA变换后的特征数目，不能大于min(n_features, n_classes - 1)，这里设置为None和1
            'store_covariance': [False, True]  # 是否存储类别的协方差矩阵，可以选择False或者True
        }
