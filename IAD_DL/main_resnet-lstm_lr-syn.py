...
from keras.layers import Input, LSTM, Dense, Flatten, Conv1D, BatchNormalization, Activation, Add, Bidirectional, Attention
...
early_stopping = EarlyStopping(monitor='loss',  # 监控的指标是验证集的损失
                               patience=100,  # 如果在 10 个 epoch 内都没有改善就停止训练
                               verbose=1,  # 显示详细信息
                               restore_best_weights=False)  # 在停止时恢复最佳权重
...
def main():
    ...
    for RATIO in RATIOS:
        ...
        # 更新参数网格（逻辑回归没有很多参数可以调整，但可以调整正则化参数C）
        param_grid = {
            'penalty': ['l2'],
            'C': [0.001, 0.01, 0.1, 1],
            'solver': ['saga'],
            'class_weight': ['balanced', None],
            'tol': [1e-3, 1e-4, 1e-5]  # 调整tol参数
        }
        ...
        # 7. 获取训练集评估数据、并保存
        y_pred_train = lr_model.predict_proba(X_train_features)
        ...
        metrics_list_all = [
            get_metrics(lr_model.predict_proba(np.squeeze(X_test_folds[i], axis=-1)), Y_test_folds[i]) for i
            in
