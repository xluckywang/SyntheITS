...
early_stopping = EarlyStopping(monitor='loss',  # 监控的指标是验证集的损失
                               patience=10,  # 如果在 10 个 epoch 内都没有改善就停止训练
                               verbose=1,  # 显示详细信息
...
def main():
    ...
    for RATIO in RATIOS:
        if DATA_TYPE == 'tightening_data':
            save_path = f"experimental_results_329/{DATA_TYPE}/{MODEL_TYPE}/experiment_syn_{RATIO:.2f}/model_reslx"
            ...
            save_path = f"experimental_results_329/validation_data/{DATA_TYPE}/{MODEL_TYPE}/experiment_syn_{RATIO:.2f}/model_reslx"  # 实验结果基本路径
            # 2. 设置样本数据的基本路径、路径字典、实验结果基本路径
        ...
        def evaluate_pca_logreg(X_train, y_train, n_components_list):
            """
            使用交叉验证评估不同n_components下的PCA降维和逻辑回归模型的性能，并输出最佳的n_components及对应的ROC AUC。

            参数:
                X_train: 训练数据特征
                y_train: 训练数据标签
                n_components_list: 一个包含不同n_components值的列表

            返回:
                best_n_components: 最佳的n_components值
                best_roc_auc: 最佳的ROC AUC值
            """
            # 初始化最佳值和最佳ROC AUC
            ...
            # 定义Pipeline，将数据标准化、PCA降维和模型拟合结合在一起
            pipe = Pipeline([
                ('scaler', scaler),
                ('pca', pca),
                ('logreg', logreg)
            ...
            # 交叉验证评估不同n_components下的模型性能（使用ROC AUC作为评估指标）
            for n_components in n_components_list:
                # 设置当前PCA的n_components值
                pipe.set_params(pca__n_components=n_components)

                # 使用交叉验证评估模型，设置评估指标为ROC AUC
                scores = cross_val_score(pipe, X_train, y_train, cv=10, scoring='roc_auc')

                # 计算平均ROC AUC
                mean_roc_auc = scores.mean()

                # 输出当前n_components值下的交叉验证结果
                print(f"PCA n_components = {n_components}, Mean ROC AUC: {mean_roc_auc:.4f}, Std: {scores.std():.4f}")
            ...
            # 输出最佳的n_components和对应的ROC AUC
            print(f"Best PCA n_components = {best_n_components}, Best Mean ROC AUC: {best_roc_auc:.4f}")

            return best_n_components, best_roc_auc

        best_n_components, _ = evaluate_pca_logreg(X_train_features, Y_train, n_components_list)

        # 进行PCA降维
        pca = PCA(n_components=best_n_components)  # 设置降维后的特征数目为16
        ...
        # 更新 GridSearchCV
        grid_search = GridSearchCV(knn_model, param_grid, cv=10, scoring='roc_auc')
