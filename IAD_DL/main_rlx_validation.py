...
import numpy as np
import pandas as pd
from classification_models.rlx import My_RESNET_LSTM
...
def main():
    ...
    for RATIO in RATIOS:
        output_directory = f'experimental_results_329/validation_data/{DATA_TYPE}/{MODEL_TYPE}/experiment_{RATIO:.2f}/exp_output/'
        save_path = f"experimental_results_329/validation_data/{DATA_TYPE}/{MODEL_TYPE}/experiment_{RATIO:.2f}/model_rlx"
        ...
        model = My_RESNET_LSTM(output_directory, input_shape, nb_classes, N_FEATURE_MAPS, verbose=True)
        ...
        metrics_list_all = [
            get_metrics(model.predict(X_test_folds[i], Y_test_folds[i], return_df_metrics=False), Y_test_folds[i]) for i
            in
