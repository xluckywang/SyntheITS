...
def visualize_filter(root_dir):
    ...
    new_feed_forward = tf.keras.backend.function(new_input_layer, new_output_layer)
...
def load_and_preprocess_data(file_path_ok, file_path_nok):
    try:
        # 从CSV文件加载数据
        data_sets_nor = pd.read_csv(file_path_ok)
        data_sets_ab = pd.read_csv(file_path_nok)

        # 数据清理，处理缺失值等
        data_sets_ab = data_sets_ab.dropna()

        # 使用sample方法对数据集进行行打乱
        data_sets_nor_shuffled = data_sets_nor.sample(frac=1).reset_index(drop=True)
        data_sets_ab_shuffled = data_sets_ab.sample(frac=1).reset_index(drop=True)

        # 合并数据集
        merged_df = pd.concat([data_sets_nor_shuffled, data_sets_ab_shuffled])

        # 将DataFrame对象转换为ndarray
        x_sets = merged_df.to_numpy()

        # 创建标签数组
        y_sets = np.zeros(x_sets.shape[0])
        y_sets[:data_sets_nor_shuffled.shape[0]] = 1
        y_sets[data_sets_nor_shuffled.shape[0]:] = 2

        y_sets = y_sets.astype(int)
...
def read_data(file_path_nor_syn, file_path_abn_syn, file_path_x_train, file_path_y_train, file_path_x_test,
              file_path_y_test, ratio=0.50):
    # 加载训练数据
    x_train = pd.read_csv(file_path_x_train)
    y_train = pd.read_csv(file_path_y_train)
    ...
    # 加载用原始训练集生成的数据
    x_train_syn, y_train_syn = load_and_preprocess_data(file_path_nor_syn, file_path_abn_syn)
    x_train_syn = pd.DataFrame(x_train_syn)
    y_train_syn = pd.DataFrame(y_train_syn)

    # 设置随机种子，保持可重复性
    np.random.seed(42)

    # 在x_train_syn上进行随机打乱
    indices_syn = np.random.permutation(len(x_train_syn))
    x_train_syn_shuffled = x_train_syn.iloc[indices_syn].reset_index(drop=True)
    y_train_syn_shuffled = y_train_syn.iloc[indices_syn].reset_index(drop=True)

    # 计算要合并的合成样本数
    total_samples = len(x_train_syn_shuffled)
    num_syn_samples = int(ratio * total_samples)

    # 选择前 num_syn_samples 个合成样本
    synthetic_samples_x = x_train_syn_shuffled.head(num_syn_samples)
    synthetic_samples_y = y_train_syn_shuffled.head(num_syn_samples)

    # 为DataFrame添加列名
    synthetic_samples_x.columns = x_train.columns
    synthetic_samples_y.columns = ["labels"]

    # 合并样本到原始数据
    x_train_new = pd.concat([x_train, synthetic_samples_x], axis=0).reset_index(drop=True)
    y_train_new = pd.concat([y_train, synthetic_samples_y], axis=0).reset_index(drop=True)

    # 将标签转换为二进制
    y_train_new = (y_train_new == 2.0).astype(int).to_numpy()
    y_test_new = (y_test == 2.0).astype(int).to_numpy()

    # 转换数据为 NumPy 数组
    data_train = x_train_new.to_numpy().reshape((x_train_new.shape[0], x_train_new.shape[1], 1))
    data_test = x_test.to_numpy().reshape((x_test.shape[0], x_test.shape[1], 1))

    # One-Hot 编码
    enc = OneHotEncoder(categories='auto')
    enc.fit(np.concatenate((y_train_new, y_test_new), axis=0).reshape(-1, 1))

    labels_train = enc.transform(np.array(y_train_new).reshape(-1, 1)).toarray()
    labels_test = enc.transform(np.array(y_test_new).reshape(-1, 1)).toarray()

    return data_train, data_test, labels_train, labels_test


def evaluate_model(model, x_data, y_labels):
    # 预测
    y_pred = model.predict(x_data)

    # 将预测结果转换为类别标签
    y_pred_labels = np.argmax(y_pred, axis=1)
    y_true_labels = np.argmax(y_labels, axis=1)
    ...
    # 计算正样本和负样本的平均预测概率
    average_positive_probability, average_negative_probability = calculate_average_prediction_score(y_labels, y_pred)

    return accuracy, precision, recall, f1, average_positive_probability, average_negative_probability


def get_metrics(y_pred, y_labels):
    # 将预测结果转换为类别标签
    y_pred_labels = np.argmax(y_pred, axis=1)
    y_true_labels = np.argmax(y_labels, axis=1)
...
def split_data_into_folds(data_test, labels_test, num_folds=5):
    # 获取数据的总数
    ...
    # 使用array_split方法分割数据
    data_folds = np.array_split(data_test, num_folds)
    labels_folds = np.array_split(labels_test, num_folds)

    return data_folds, labels_folds


def calculate_average_prediction_score(y_true, y_pred):
    # 获取第一列元素值为1的行索引
    positive_indices = np.where(y_true[:, 0] == 1)[0]
    negative_indices = np.where(y_true[:, 1] == 1)[0]

    # 从y_pred_pro中提取对应的预测概率
    positive_probabilities = y_pred[positive_indices]  # 假设第一列是正类别的概率
    negative_probabilities = y_pred[negative_indices]  # 假设第二列是负类别的概率

    # 计算平均概率
    average_positive_probability = np.mean(positive_probabilities[:, 0])
    average_negative_probability = np.mean(negative_probabilities[:, 1])

    return average_positive_probability, average_negative_probability


def read_data_syn(file_path_nor_syn, file_path_abn_syn, file_path_x_train, file_path_y_train, file_path_x_test,
                  file_path_y_test, ratio=0.50):
    # 加载训练数据
    x_train = pd.read_csv(file_path_x_train)
    y_train = pd.read_csv(file_path_y_train)
    ...
    # 合并样本到原始数据
    x_train_new = synthetic_samples_x.reset_index(drop=True)
    y_train_new = synthetic_samples_y.reset_index(drop=True)
