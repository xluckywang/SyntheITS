...
from keras.layers import Input, LSTM, Dense, Dropout, Conv1D, BatchNormalization, Activation, Add, Bidirectional, \
    Attention
...
#                                patience=1000,  # 如果在 10 个 epoch 内都没有改善就停止训练
#                                verbose=1,  # 显示详细信息
...
def main():
    ...
    MODEL_TYPE = "timediffusion"  # "timegan_and_timediffusion" doppelganger
    RATIOS = [1.00]
    ...
    for RATIO in RATIOS:
        ...
        def residual_block(x, filters, kernel_size, dropout_rate, activation='relu'):
            # First Conv1D layer
            x_res = Conv1D(filters, kernel_size, padding='same')(x)
            x_res = BatchNormalization()(x_res)
            x_res = Activation(activation)(x_res)
            x_res = Dropout(dropout_rate)(x_res)  # 添加 Dropout
            ...
            x = Dropout(dropout_rate)(x)  # 添加 Dropout

            return x

        def build_resnet1d_model(input_shape, num_blocks=1, filters=64, kernel_size=3, lstm_units=64, dropout_rate=0.2,
                                 num_heads=8):
            ...
            self_attention = MultiHeadAttention(num_heads=num_heads, key_dim=lstm_units // num_heads)(x, x)
            self_attention = Dropout(dropout_rate)(self_attention)

            # Multi-Head Attention layer
            multi_head_attention = MultiHeadAttention(num_heads=num_heads, key_dim=lstm_units // num_heads)(
            ...
            x = Reshape((num_heads * (2*lstm_units // num_heads),))(pooled)
            x = Dropout(dropout_rate)(x)  # 添加 Dropout
        ...
        # 编译模型
        model = build_resnet1d_model(input_shape, dropout_rate=0.1)
