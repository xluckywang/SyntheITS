...
# TOD # 使用格拉姆角场绘制图像 -----GASF
def calculate_gramian_matrix(X):
    return np.outer(X, X) - np.outer(np.sqrt(1 - X**2), np.sqrt(1 - X**2))
...
def plot_and_get_curves(data, color='green'):
    ...
    for index, row in data.iterrows():
        ...
        row_data_normalized = ((row_data_norm - np.max(row_data_norm)) + (row_data_norm - np.min(row_data_norm))) / (np.max(row_data_norm) + np.min(row_data_norm))
